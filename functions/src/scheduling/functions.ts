...
import { getConnection, createConnection } from '../database';
import { AlertConditionType, ComparisonOperator, NotificationChannel } from './types';
import { sendNotification } from './notifications';
import { parseAndFormatCronExpression } from '../utils/cronExpression';

// 스케줄링된 쿼리 실행
export const executeScheduledQuery = functions.pubsub
    ...
    try {
      ...
      const promises = scheduledQuerySnapshot.docs.map(async (doc) => {
        ...
        try {
          ...
          // 7. 알림 조건 확인 및 알림 생성
          if (query.notifications?.enabled) {
...
// 특정 스케줄링된 쿼리 강제 실행
export const manualExecuteScheduledQuery = functions.https.onCall(async (data, context) => {
  // 인증 확인
  if (!context.auth) {
  ...
  if (!scheduledQueryId) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Scheduled query ID is required'
    );
  }
...
export const cleanupScheduledQueryExecutions = functions.pubsub
...
function shouldExecuteNow(query, now) {
  ...
  switch (frequency) {
    ...
    case 'HOURLY':
      ...
      return (
        now - lastExecution >= hourInterval &&
    ...
    case 'WEEKLY':
      ...
      return (
        now - lastExecution >= weekInterval &&
...
function isCronMatch(cronExpression, date) {
  try {
    ...
    // 각 부분이 일치하는지 확인
    return (
      isFieldMatch(minute, currMinute, 0, 59) &&
...
async function executeQuery(query) {
  ...
  // 연결 정보 조회
  const connectionData = await getConnection(query.createdBy, connectionId);
      ...
      case AlertConditionType.CUSTOM_CONDITION:
        ...
        if (condition.columnName && condition.operator && results.rows && results.rows.length > 0) {
          ...
          if (columnValues.length > 0) {
            // 값들이 조건을 만족하는지 확인
            const someMatch = columnValues.some(value => 
...
async function createAndSendNotification(query, executionId, results, alertInfo) {
  try {
    ...
    const notificationType = alertInfo.triggered ? 
      (condition.type === AlertConditionType.ERROR ? 
        'QUERY_EXECUTION_ERROR' : 'QUERY_EXECUTION_ALERT') : 
      'QUERY_EXECUTION_SUCCESS';
    
    // 알림 생성 및 전송
    const notificationChannels = notifications.channels || [NotificationChannel.EMAIL];
