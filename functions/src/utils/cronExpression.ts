...
export function parseAndFormatCronExpression(cronExpression: string): string {
  try {
    // 기본 유효성 검사
    if (!cronExpression) {
...
// CRON 표현식의 개별 부분 검증
function validateCronPart(part: string, range: { name: string, min: number, max: number }): string {
