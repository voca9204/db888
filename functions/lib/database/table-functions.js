...
exports.deleteTableRow = exports.insertTableRow = exports.updateTableRow = exports.getTableData = void 0;
const functions = require("firebase-functions");
const index_1 = require("./index");
// Cloud Function to get table data with pagination and sorting
exports.getTableData = functions.https.onCall(async (data, context) => {
    // Ensure the user is authenticated
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required");
    }
    const userId = context.auth.uid;
    const { connectionId, tableName, page = 1, pageSize = 50, sortColumn, sortDirection, filters = [] } = data;
    if (!connectionId || !tableName) {
        throw new functions.https.HttpsError("invalid-argument", "Connection ID and table name are required");
    }
    try {
...
exports.updateTableRow = functions.https.onCall(async (data, context) => {
    // Ensure the user is authenticated
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required");
    }
    ...
    try {
        ...
        if (columns.length === 0) {
            throw new functions.https.HttpsError("invalid-argument", "No columns to update");
        }
...
exports.insertTableRow = functions.https.onCall(async (data, context) => {
    // Ensure the user is authenticated
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required");
    }
    ...
    try {
        ...
        if (columns.length === 0) {
            throw new functions.https.HttpsError("invalid-argument", "No columns to insert");
        }
...
exports.deleteTableRow = functions.https.onCall(async (data, context) => {
    // Ensure the user is authenticated
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Authentication required");
    }
...
//# sourceMappingURL=table-functions.js.map